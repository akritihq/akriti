...
Two constraints to carry in when it is: `numpy` is a lazy, function-scoped
import inside `save`/`load` only, raising a clear `ImportError` naming it if
absent (§3.3) — it is not a declared dependency, see `DEPENDENCIES.md` — and
requirement 1 now has two clauses, `load(dump(d)) == d` *and*
