...
durable claim is that CSV costs ~2.1x the bytes and takes ~1.2 s where `.npz`
takes ~0.01 s: two orders of magnitude on load.** D12 carries why a ratio drawn
from this table does not transfer and the appendix records absolute times
instead.
...
- **2026-08-09 (47)** — 13 normative defects. Normative: §6.3's `allclose` matches `+inf` deaths outside the tolerance formula; §5's `finitize` rejects any substituted death below an essential bar's birth, in both substituting modes, and `at="max_finite_death"` on an all-essential diagram; B1 is restated against `metas`; **B8** states `DiagramBatch` immutability; **I8** requires public construction not to alias a caller's arrays or `metas`, and documents the subscript assignment nothing in scope can prevent; `from_gudhi` and `from_array` take a keyword-only `dim=`, and `from_array` a `columns=` sequence naming its input's columns; §8 requires `filtration` where the adapter's input form determines it, excludes non-finite floats from `params`/`provenance`, and defines what `same_provenance` compares, `description` excluded; §9.1 carries the essential-bar matching and combination rules; §10.1 requirement 1 binds `DiagramBatch`, its `==` clause scoped to NumPy-backed objects; requirement 2 generalises to any lazily-imported extra and replaces §3.3's rule naming two filenames; §10.2 specifies the `meta.json` schema and a `load` that dispatches on `kind` and rejects an unrecognised `spec`; §10.3 refuses `diagram_id` and moves the exporters to `adapters.py`; §11.2 gains the batch round-trip, `load`-dispatch and `allclose`-on-essential-bars cases. Scope: **extended persistence is a §1 non-goal** — no essential classes, four sub-diagrams, I6 violated only by the relative and extended− bars — with §11 rejecting the 4-tuple and documenting the member that arrives undetected. Opened and resolved **D19**, §9.1's departure from the delegation rule; sixteen decisions, all settled. Also: `DiagramMeta.space` becomes `description`, the header gains a **Version** row, a `dump()` that never existed is spelled `save`/`load`, and mathematical notation is LaTeX throughout.
- **2026-08-10 (48)** — Fifth review pass, on contradictions and stale facts rather than defects. Normative in one place: `provenance["essential_bars"]` becomes a closed four-value enum, the substituted death moving out of the string into a numeric `essential_bars_finitized_at`, both validated at `DiagramMeta` construction (§5, §8). Corrections: A.3's `float32`-eps ratio is ~7.5, not ~71; A.6's scaling claim weakens to "at least linearly" and withdraws a direction for what happens above H0, two rows over two different spaces supporting neither; A.6's per-degree column is a median, not a breakdown; Appendix A's preamble scopes its environment to A.1-A.4; entry 42 drops the load multipliers entry 44 retired; §4.2's `__getitem__` spells `int(offsets[i])`; §3.3's scoping sentence stops asserting that its measurements were measured; §9.2's install-rate and commit-count claims cite public sources, entry 45 having removed the internal one. Editorial: `dims` gains its dtype comment; §3.2's and §4.3's catalogues gain the stored fields; `to_arrays()` returns `dict[int, Array]`; ratio signs are ASCII; the header table gains header cells and records M0 as met. Condensation, on the history document's contract that it carries the argument this one does not need: §8's unreachable chained-`finitize` illustration, A.6's sentence restating its own column label, four pull-request references and a conference-scheduling aside.
- **2026-08-10 (49)** — Residuals of entries 47 and 48, found by re-reading those passes against the document rather than against their own changelog entries. Normative in two places, both on `from_array`'s `columns` (§10.3): it MUST name `birth` and `death` exactly once each and `dim` at most once — a repeated name and a missing one being one defect from two ends, neither resolvable by the position the argument exists to override — and MUST raise on the argument before `arr` is inspected; and §11.2 gains the `to_csv()`/`from_array` round trip through a header row, plus the five invalid-`columns` cases, on an `arr` that would construct cleanly read positionally. Entry 47 added the whole mechanism with nothing in §11.2 behind it, which is the gap that entry's own last finding names one clause over. §11 states that `columns`, where supplied, and not the column count is what answers its degree question, the two agreeing by the new rule. Corrections: §12.2's D17 cell cited "§8's opening sentence" twice for a claim entry 47 deleted from §8 — once inside the recorded question, once as a live cross-reference — and both now stand on what §8's opening paragraph says. History document: entry 46 gains the expansion every other entry has, including the conference-schedule justification entry 48 struck from it; ratio signs are ASCII there too, entry 48 having normalised the body only.
- **2026-08-10 (50)** — Condensation pass. No requirement changed, and the five BCP 14 keyword counts are identical before and after. Out: §9.2's install rate and market framing, D5's goodwill clause, §1's staffing fact, and the two citations of an internal policy file that does not publish alongside this document, at §9.1 and D19, each replaced by the rule it was citing on entry 45's precedent; six passages narrating this document's own drafting (§3.3 twice, §5, §9.3, §10.2, §12); D12's restatement of the requirement-5 argument §10.1 carries, and §11's of the single-writer argument §5 carries; A.5's and A.6's closing interpretation, which §9.3 and D12 carry. §9.2's `pypistats` citation moves up to the claim that needs a source.
