...
- **Public construction MUST NOT alias an array a caller can write to.**
  Stated over writability rather than over copying, because the copy is the
  mechanism and not the requirement: a JAX array is immutable, so a caller
  who retains one cannot write through it, and a conforming implementation
  MAY skip the copy wherever the backend's arrays are immutable. The rule is
  otherwise unchanged, and what follows is what it protects. This is
  ...
  `DiagramBatch.from_diagrams` — MUST therefore copy any array it is given
  that the caller could still write to, rather than store it. On a backend
  whose arrays are immutable that set is empty and the copy is dead cost; on
  every other backend it is all of them, which is why an implementation
  targeting one backend can and should simply copy. The cost is one copy at a
  boundary that is already
...
diagram's bars, and §8.1 specifies no cache, so "already available" means the
caller may have one rather than that this type keeps one — and it localizes a
mismatch: if two batches' hashes
...
names, written in this order. **This section says "member" only in the zip
sense** — one named entry of an archive, of which an `.akd` has exactly two
and the `bars.npz` inside it has three or four. The `bars.npz` entries are
called **arrays** throughout, and the diagrams of a `DiagramBatch` are called
**diagrams**; §8.2's "member hash" is the third sense and belongs to that
section, where no zip exists. `inf` lives in `bars.npz`, where NumPy
...
- **2026-08-23 (68)** — **Tier 1 of a full review pass read against `src/akriti/diagrams/`: six corrections, each a place this document was wrong rather than thin.** §2's "the diagonal is implicit and MUST NOT be stored" read as a prohibition on `birth == death` and so forbade the trivial bars §4's padding argument, §11.1's three `strip_padding` modes and §11.2's required test case are all about; the prohibition now names the diagonal-as-a-multiset. §4.2's `__getitem__` sketch read `offsets[i]` and `offsets[i + 1]` without normalising, so `i = -1` read `offsets[-1]` and `offsets[0]` and returned an **empty** diagram carrying the last diagram's metadata — silently, in the operation §4.3 routes every batch accessor through. The sketch normalises, four rules follow it, and §4's interface block gains `__iter__`, which §4.3 already argued from. §3.2 gains a general `meta` propagation rule for derived diagrams — `canonical()` had the only one — and with it the requirement that `d.finite` record the drop it performs on `finitize(at="drop")`'s terms, the two producing the same diagram bar for bar; §3.2 also states why `d.dim(k)` is not that case. **`core.py` moved first here**, `d.finite` having actively produced the contradiction while §3.2 was merely silent. §12's header loses "; §12.1 is empty", which contradicted the same sentence.
- **2026-08-23 (69)** — **§11's `reduced_homology=False` impossibility check, and Appendix A's own provenance — both settled by measurement rather than by reading.** The check tested "all H0 deaths are finite", a reduction over an empty selection and so vacuously true of any diagram with no degree-0 row. Scoping it to non-empty diagrams does not exclude that: the diagram is non-empty and its *H0 sub-diagram* is what is missing. Measured in the pinned environment §9.2 forces — `homology_dimensions=(1, 2)`, `(1,)` and `(2,)` each return a non-empty array with no degree-0 row — so the unscoped check refuses three ordinary calls. The predicate is now three-termed and **new A.10** records the run with both controls, `rfcs/evidence/giotto_h0_scope.py` reproducing it. **The review's Appendix A finding was wrong, and what replaced it is worse in one place.** `rfcs/evidence/probe_backends.py` has carried a scikit-learn shim, `patch_giotto()`, since the document's first commit, so giotto-tda 0.6.2 *does* run on scikit-learn 1.8.0 under it and the preamble never contradicted §9.2. Confirmed by rebuilding that environment: all six A.1 giotto rows and all of A.2 reproduce, and reproduce identically again in an unshimmed pinned environment. **The document had never mentioned the shim**, which the preamble now states; A.1's note drops a causal claim that is measurably false. §9.2 is corrected on a point nobody raised: "unusable on a default `pip install` of its own declared dependency" is not defensible, giotto-tda 0.6.2 pinning `scikit-learn==1.3.2` exactly, so the default install works. What is true is that it cannot be installed alongside a current scikit-learn at all. #726 stands as filed, the traceback being unchanged.
- **2026-08-23 (70)** — **D23 settled, D24 opened, and two clauses the implementation had been carrying alone.** **D23**: §3.3 promised a JAX-backed diagram while JAX's defaults supply neither dtype the type requires. New **A.11** corrects the premise the question was raised on — JAX carries a *second*, narrower lever, `jax_explicit_x64_dtypes='allow'`, under which an explicitly requested `float64` and `int64` are honoured with `jax_enable_x64` still off and the process default still `float32`, and both types then construct. So the diagram is constructible and a *default* install is what cannot build one. §3.3 states it on D16's pattern and forbids this library from setting either flag: both are process-global and neither has a public scoped form. `tests/test_rfc0001_jax_live.py` exercises it, marked and skipped where JAX is absent; JAX does not enter the closure. **D24** records what this pass would not decide: §10.1 requirement 4 is defeated by `meta.json`'s own `spec_version` under a bump rule that fires on any BCP 14 keyword being altered. Three coherent answers, with the project lead; **the current rule stands unchanged meanwhile**. §3.1 additionally states that derivation from an already-valid instance MAY skip revalidation, without which §3.2 and §7 are unimplementable — validating I6 exactly is a branch on array values — and that I8's copy rule binds `params` and `provenance`, not only the arrays and `metas`.
- **2026-08-23 (71)** — **Eleven specification gaps, then the over-claims and the editorial line.** `clamped_rows` gets a clamp target, a recording obligation and its unfixed threshold stated as a gap with the interoperability cost named (§3.1). `namespace_of`'s sketch gains the numpy-floor branch, without which a `numpy` 1.26 array is told to install `akriti[torch]` (§3.3). `d.dimensions` joins the eager-only enumeration. `to_csv()` is stated to have no reader half rather than implying one (§10.3). `from_gudhi` says that it takes one sample where `fit_transform` returns many, with the asymmetry against `from_giotto` explained (§11). §4.2's "MUST return a view" becomes a prohibition on deep-copying, JAX slicing allocating and D23 having just said JAX-backed batches exist. §9.1's bottleneck definition takes its bijection **within each degree** rather than importing "same dimension" a paragraph later — the defect that section attributes to persim — and states the `inf - inf = 0` convention. `eq=False` and unhashability are specified (§3.1); §8.1's per-element `>d`/`>i` packing is specified and signed-zero normalisation placed before either byte path, where §11.2's agreement test needs it; §8 defers to §11 for the per-adapter obligation, the single-point-of-truth failure that produced D17; and `save()` MUST refuse a non-host-resident array by name rather than attempt a transfer its owner did not ask for. **Then**: `b.canonical()`'s eager-only status is stated as a property of routing rather than of the operation, `searchsorted` giving a traceable form; D21's cost cell names the common giotto configuration rather than the rare deliberate one; §11.1 states why `strip_padding` may default-and-warn where §5.1 and D21 refuse to; §3.2 states that `d.essential` and `d.finite` are not complements. Seven editorial corrections, `F30a`-`F30g`, including B8 gaining an explicit permission for derived caches and §11.2's determinism case asserting the pinned `ZipInfo` fields directly rather than sleeping 2.5 s per case.
- **2026-08-23 (72)** — **New Appendix D, the normative-requirements index, and the internal references swept.** 212 BCP 14 clauses across ~44,000 words cannot be reviewed for *consistency* by reading, and four of this pass's findings were one failure repeated: a rule argued in one section and not propagated to the places it binds. The index puts every clause about one subject together, and it is **generated** — `tools/normative_index.py`, with `tests/test_rfc0001_normative_index.py` failing when the body and the appendix disagree — on D15's own ground that a separately maintained index is a cached answer to an always-computable question. Three exclusions are pinned by their own tests, a count check being unable to tell a use from a mention: fenced code, lowercase keywords, and §12's rows, which the section's own header calls pointers. The appendix is non-normative; where a row and the body differ the body governs. §1 gains a table naming `core/`, `castle/`, `compat/`, `repro/` and the rest, so that goal 2's claim that no reader need adopt the rest of Akriti survives contact with the document; §4's opening sentence is rewritten to stand without them.
- **2026-08-23 (73)** — **The document becomes 1.1.0.** The minor moves under §10.2's rule as it currently stands: entries 68 through 72 add, remove and alter many clauses carrying BCP 14 keywords. That the rule itself is under review as **D24** does not license taking a smaller increment under it — a document that suspended its own version rule while deciding whether to change it would be doing what D24 exists to prevent. One bump for the whole pass, not one per entry. `io.py`'s `_SPEC_VERSION` and the four `spec_version` pins in the I/O tests follow.

---

## Appendix D — Normative requirements index
...
212 clauses: MAY 10, MUST 169, MUST NOT 31, SHOULD 2.
...
| `N2-2` | §2 | **MUST NOT** | *Finite*: infinite diagrams (e.g. the full diagonal) are out of scope, and the diagonal is not materialised as a multiset: a conforming representation MUST NOT store it, and nothing in this document adds a diagonal bar to a diagram it did not receive. |
...
| `N3.1-4` | §3.1 | **MUST** | - No method of either type may mutate. Every method that looks like a mutation (`finitize`, anything in §3.2) MUST construct and return a new `PersistenceDiagram` rather than modify `self`. |
| `N3.1-5` | §3.1 | **SHOULD** | `DiagramBatch` is under the same rule, stated as B8 (§4.2): its buffers *are* every member diagram's arrays, so a batch-level in-place write is a write to N diagrams at once. - `@dataclass(frozen=True)` is the mechanism, and it is not sufficient by itself. Both types SHOULD be frozen, as `DiagramMeta` already is (§8). |
...
| `N3.1-7` | §3.1 | **MUST NOT** | The array API standard supplies nothing that closes the gap, having considered it and declined. It specifies no read-only array, no writeability flag and no immutable view; it lists `__setitem__` among the array object's methods; and its copies-views-and-mutation topic records read-only views as a rejected option, "hard to implement" and a backward-compatibility problem "for current strided array libraries", offering in their place the advice to avoid mutating anything that might be a view. - Public construction MUST NOT alias an array a caller can write to. Stated over writability rather than over copying, because the copy is the mechanism and not the requirement: a JAX array is immutable, so a caller who retains one cannot write through it, and a conforming implementation MAY skip the copy wherever the backend's arrays are immutable. |
...
| `N3.1-9` | §3.1 | **MUST** | `DiagramBatch` MUST copy the `metas` sequence on the same rule, B1 (§4.2) being stated over its length: a caller who appends to a list they passed in would otherwise break an invariant after the construction that enforced it. |
| `N3.1-10` | §3.1 | **MUST** | `DiagramMeta` MUST take the same rule for `params` and `provenance`, which are `Mapping[str, Any]` and which frozenness does not reach any further than it reaches into an array: |
| `N3.1-11` | §3.1 | **MUST** | Unlike the array case this is enforceable — the mappings hold JSON-representable values (§8), which are copyable — so `DiagramMeta` MUST copy them, recursively over nested lists and mappings, and MUST expose them read-only. |
| `N3.1-12` | §3.1 | **MUST NOT** | `DiagramBatch.__getitem__` is the deliberate exception to the third rule and MUST NOT be implemented through the public path. It aliases on purpose (§4.2) and is safe for precisely the reason a caller-supplied array is not: both ends of the alias are objects this document forbids anyone to write to, and neither is reachable by a caller who held a reference before the batch existed. |
| `N3.1-13` | §3.1 | **MAY** | Derivation from an already-valid instance MAY skip revalidation, and this is a general rule rather than that one exception. "Enforce at construction" above is an obligation on the *public* paths — the constructor, the adapters, `from_diagrams`. |
| `N3.1-14` | §3.1 | **MUST** | The bypass is sound under stated conditions, and MUST be confined to them. |
...
| `N3.3-6` | §3.3 | **MUST** | - `numpy` absent — MUST raise a clear `ImportError`. - `numpy` present but older than 2.0 — MUST raise a clear `ImportError` as well, rather than proceeding into an `AttributeError` on the first array-API call. |
| `N3.3-7` | §3.3 | **MUST** | Both serialization messages MUST name the extra — "install `akriti[io]`" — not the bare package. |
| `N3.3-8` | §3.3 | **MUST** | The row-sequence adapter fallback MUST instead name `akriti[numpy]`, the extra for the namespace it needs. |
| `N3.3-9` | §3.3 | **MUST NOT** | Adapters preserve the input namespace. `from_*` MUST NOT force-convert to NumPy. |
| `N3.3-10` | §3.3 | **MUST NOT** | JAX is supported only in a 64-bit configuration the caller sets, and this document MUST NOT set it (D23). JAX's defaults supply neither dtype the diagram type requires: an explicitly requested `float64` is truncated to `float32` with a `UserWarning`, so I2 rejects the bars, and an explicitly requested `int64` is truncated to `int32`, so B7 would reject `offsets` if I2 had not already raised. |
| `N3.3-11` | §3.3 | **MUST NOT** | `core.py`, `adapters.py` and `io.py` MUST NOT set either flag. Both are process-global, neither has a public scoped form, and setting one would silently change the numerics of unrelated JAX code in the caller's program — a library reaching outside its own call to make its own invariant hold. |
| `N3.3-12` | §3.3 | **MUST** | The failure a caller sees is I2's ordinary `ValueError`, which names the dtype it got; §3.3 is where it is explained, and the error MUST point here. |
| `N3.3-13` | §3.3 | **MUST** | The constraint MUST be exercised rather than asserted, on D16's pattern and for §9's reason: a support claim no test runs is a claim that rots silently. |
| `N3.3-14` | §3.3 | **MUST** | A `@pytest.mark.backend` test MUST construct a diagram and a batch under `jax_explicit_x64_dtypes='allow'` and assert that the default configuration refuses them, skipping where JAX is absent — JAX is not in the default test environment and does not enter the dependency closure to satisfy this. |
| `N3.3-15` | §3.3 | **MUST** | Namespace resolution MUST go through exactly one function, and its answer MUST depend on the input and never on the environment. `d.xp`, I7, B5 and §4.2's `from_diagrams` check are all defined over what that one function returns: |
| `N3.3-16` | §3.3 | **MUST** | The branch MUST test the type's own module rather than importing `numpy`, which this scope may not do, and MUST name `akriti[numpy]`, on the same rule the messages above state. |
| `N3.3-17` | §3.3 | **MUST** | The native method MUST be preferred wherever it exists, and the fallback MUST be reached only where it does not. |
| `N3.3-18` | §3.3 | **MUST** | `array-api-compat` MUST therefore be declared in the `akriti[torch]` extra with a version floor, on the terms §10.1 requirement 2 sets. |
| `N3.3-19` | §3.3 | **MUST** | The import MUST be lazy and function-scoped. |
| `N3.3-20` | §3.3 | **MUST** | A test MUST therefore assert that `essential`, `persistence`, `bar_counts`, `dim(k)` and `finite` return the same values under two namespaces for the same bars, and until it runs against torch, a torch-backed diagram is namespace-correct and not established as object-correct. |
| `N3.3-21` | §3.3 | **MUST** | The constraint MUST be verified in CI, not assumed. A test MUST assert, for every supported backend that implements `__array_namespace__` natively, that the method called on two separate arrays of that backend returns the same object. |
| `N3.3-22` | §3.3 | **MUST** | What MUST be pinned for those is which branch of the resolution rule they take. A test MUST assert that a `torch.Tensor` does not implement `__array_namespace__` and therefore resolves through `array_api_compat`, marked `@pytest.mark.backend` on the `akriti[torch]` extra. gh-58743 is open and the attribute is being withheld until near-full conformance, which means it lands eventually; on the release that adds it, a torch tensor stops resolving through the wrapper and `d.xp` changes from `array_api_compat.torch` to `torch`, so two diagrams built under adjacent torch versions fail the identity comparison while being the same kind of thing. |
| `N3.3-23` | §3.3 | **MUST** | That MUST break the build rather than reach a user, and when it does, torch enters the identity test above on the same terms as every other backend. |
...
| `N4.2-4` | §4.2 | **MUST** | - A non-integer index MUST raise `TypeError`, through `operator.index` rather than an `isinstance` check, so that any object implementing `__index__` works. |
| `N4.2-5` | §4.2 | **MUST** | That is also what refuses a `slice`. - A negative index MUST be normalised to `i + len(batch)` before either `offsets` read. - An index still outside `range(len(batch))` after normalisation MUST raise `IndexError`, naming the index the caller passed rather than the normalised form: an `IndexError(-3)` raised for a `batch[-5]` names an index nobody asked for and sends the reader after the wrong bug. - Slicing a batch is not supported. |
| `N4.2-6` | §4.2 | **MUST** | `DiagramBatch` MUST be iterable, yielding its diagrams in batch order. §4.3's `[d.dimensions for d in b]` already depends on it. |
| `N4.2-7` | §4.2 | **MUST** | Iteration is eager-only for the reason `__getitem__` is (§3.3), and re-iterating a batch MUST yield the same diagrams again. |
| `N4.2-8` | §4.2 | **MUST NOT** | `__getitem__` MUST NOT deep-copy the buffers: it MUST construct its `PersistenceDiagram` from ordinary slices of the batch's own `dims`, `births` and `deaths`, and MUST NOT materialise an independent copy of the bar data. |
| `N4.2-9` | §4.2 | **MUST** | Whether the slice is a view is the backend's business, not this document's — it is on NumPy and torch, and on JAX slicing allocates, so a clause reading "MUST return a view" is one no JAX-backed batch could satisfy and D23 has just finished saying JAX-backed batches exist. |
| `N4.2-10` | §4.2 | **MUST** | `offsets` has its own invariants, and `core.py` MUST enforce them at `DiagramBatch` construction, the same way §3.1 enforces I1 through I9 for a single diagram: |
| `N4.2-11` | §4.2 | **MAY** | An implementation MAY therefore memoise a value derived from the stored fields — the `offsets` bounds as Python ints being the case — provided the cache is written once, is a pure function of fields B8 already forbids anyone to change, and is never observable through the public surface. |
| `N4.2-12` | §4.2 | **MUST** | `DiagramBatch` MUST provide one: |
| `N4.2-13` | §4.2 | **MUST** | Every input diagram MUST share one array namespace, and `from_diagrams` MUST check it. I7 constrains the three arrays *within* one diagram and says nothing across diagrams, so a sequence mixing a JAX-backed diagram with a NumPy-backed one satisfies I7 term by term and still cannot become a batch. |
| `N4.2-14` | §4.2 | **MUST** | But there is no diagram to derive a namespace from, and this type has no default backend to fall back on (§3.3 is explicit that NumPy is not one), so the namespace MUST come from the caller in that one case: |
| `N4.2-15` | §4.2 | **MUST** | Passing `xp` alongside a non-empty `diagrams` is permitted and MUST be rejected with `ValueError` if it disagrees with the diagrams' own namespace, compared by `is` and not by any weaker test — D16 requires namespace identity of every supported backend and this is one of the four comparisons it names, alongside I7, B5 and the check one paragraph above. |
...
| `N7-1` | §7 | **MUST** | - Every consumer MUST treat row order as arbitrary. - Equality (§6.3) MUST be computed on the multiset. - Adapters MUST NOT sort. |
| `N7-2` | §7 | **MUST NOT** | It MUST NOT be implemented with `lexsort`. That function is not part of the array API standard (§3.3). |
| `N7-3` | §7 | **MUST NOT** | `canonical()` is a presentation concern and MUST NOT be assumed by any numerical routine. |
| `N7-4` | §7 | **MUST** | `DiagramBatch.canonical()` MUST instead sort within each segment independently — `[offsets[i], offsets[i+1])` for every `i` — and MUST NOT reorder the segments themselves. |
...
| `N8.1-1` | §8.1 | **MAY** | - A namespace whose array exposes a C-contiguous buffer of exactly the expected format and item size MAY reinterpret the raw machine values and byte-swap the block, which for an IEEE 754 `double` or a two's-complement `int32` is by definition what the per-element form emits. - Every other namespace MUST pack element by element as `>d` for `births` and `deaths` and `>i` for `dims`, in canonical order, reading each element as a Python `float` or `int`. |
| `N8.1-2` | §8.1 | **MUST** | Negative zero MUST be normalised to `+0.0` before hashing. `-0.0 == 0.0` is true, so §6.3's `==` calls two diagrams differing only in the sign of a zero equal, and §7's canonical order cannot separate them either: a stable sort leaves numerically equal keys in the order they arrived. |
...
| `N9.1-5` | §9.1 | **MUST NOT** | An implementation MUST NOT reach that value by subtracting the deaths. |
| `N9.2-1` | §9.2 | **MUST** | - `from_giotto` MUST be tested against *stored fixture arrays*, not a live giotto call, or CI will fail for reasons that have nothing to do with us. |
| `N9.2-2` | §9.2 | **MUST NOT** | Fixtures are committed with the giotto and scikit-learn versions that produced them. - giotto-tda MUST NOT enter the default dependency closure. |
...
| `N9.2-4` | §9.2 | **MUST NOT** | No giotto source has been read, and MUST NOT be read while implementing `compat/`.* |
...
| `N10.2-1` | §10.2 | **MUST** | An `.akd` archive MUST contain exactly these two members, under exactly these names, written in this order. |
...
| `N10.3-3` | §10.3 | **MUST** | §11.2's round-trip case for this pair is therefore `to_csv` then a caller-side parse then `from_array`, and it needs `akriti[numpy]` to run; it MUST say so rather than reading as a test of an API that does not exist. - `to_parquet()` → a `pyarrow.Table` with the same three columns and order as `to_csv()` (`dim` int32, `birth`/`death` float64), so `inf` round-trips exactly — Parquet's `double` is IEEE 754, unlike JSON's. |
...
| `N11.2-3` | §11.2 | **MUST** | - A diagram with essential bars (GUDHI, Ripser). - An empty diagram, and a diagram empty in one degree but not another. - A diagram with repeated identical bars — multiplicity MUST survive. - A diagram with a genuine zero-persistence bar. - Cross-backend agreement GUDHI vs Ripser on the same point cloud, with an explicit `rtol=1e-6` and a comment pointing at §6.2. |
| `N11.2-4` | §11.2 | **MUST** | The coefficient field MUST be pinned explicitly on both sides, with a comment pointing at §9.3: the two backends default to different fields, so a comparison taking the defaults is matching bars across two different homology theories. - The two coefficient-field defaults §9.3 tabulates, asserted against the installed backend — GUDHI 11, Ripser 2 — one test per backend, marked `@pytest.mark.backend`. §11 writes these numbers into diagrams that stated no field, so an upstream change to either MUST break the build rather than reach a user's provenance. - `coeff_field` and `provenance["coeff_field_source"]` recorded by `from_gudhi` and `from_ripser` in both directions (§11): passing the field gives `"caller"` and the value passed; omitting it gives `"backend_default"` and that backend's default. |
| `N11.2-5` | §11.2 | **MUST** | A suite testing only the omitted case passes on an adapter that ignores the argument outright. - `from_giotto`'s two required arguments, refused on the argument rather than on the data (§11): omitting `reduced_homology` or `infinity_values` MUST raise `TypeError`, and `infinity_values=None` — giotto's own default, which resolves to the transformer's cutoff and so writes a finite sentinel whenever that cutoff is finite — MUST raise `ValueError` naming the default and the cutoff together, as MUST any finite value. |
| `N11.2-6` | §11.2 | **MUST** | The refusal cases MUST run against real giotto output captured with that default, since that array is the one the sentinel is actually in; a suite that exercises them only on a hand-written array proves the check fires but not that it fires on the input it exists for. |
| `N11.2-7` | §11.2 | **MUST NOT** | Where a capture predates the requirement and therefore carries the sentinel, the `"faithful"` label MUST NOT be asserted over it — the recapture is the fix, not a widened adapter. - `allclose` on two diagrams that are within tolerance of each other but whose canonical orders differ because of that tolerance, asserting `True`. |
| `N11.2-8` | §11.2 | **MUST** | The mechanism MUST be asserted directly — every member's `ZipInfo.date_time` is the zip epoch, `(1980, 1, 1, 0, 0, 0)`, and its `compress_type` is `ZIP_STORED` — rather than only inferred from two writes agreeing. |
| `N11.2-9` | §11.2 | **MAY** | A second write MAY still be compared, but it is a corroboration rather than the test. - A `DiagramBatch` round-trip, as its own case — both clauses of §10.1 requirement 1, with `save(b, p)`, `load(p) == b` and `load(p).same_provenance(b)`. |
| `N11.2-10` | §11.2 | **MUST** | The suite MUST cover a batch whose diagrams have different bar counts, so `offsets` is exercised rather than degenerate; one containing an empty diagram, so a zero-length segment is; one of length zero (§4.2's `xp=` constructor); and one whose diagrams are in an order that no sort would produce, since both comparisons are order-sensitive across diagrams (§6.3, §8) and a `load` that recovered every diagram into the wrong slot passes a test built from identical members. - `load` dispatches on `kind`, not on the payload (§10.2): a saved diagram loads as a `PersistenceDiagram` and a saved batch as a `DiagramBatch`, including a length-one batch, which holds exactly the bars its member does and is the case an `offsets`-sniffing implementation gets wrong. |
| `N11.2-11` | §11.2 | **MUST** | A file whose `format` string, `format_version` or `kind` is unrecognised, and one whose `kind` disagrees with the members present, MUST each raise rather than load. - The `to_csv()` / `from_array` round trip, through the header row (§10.3): writing a diagram with `to_csv()` and reading the file back with `columns` taken from its header MUST reproduce the bars, `dim` included. |
| `N11.2-12` | §11.2 | **MUST** | The validation rules MUST be tested on the argument rather than only through a valid call — a `columns` of the wrong length, one carrying an unrecognised name, one naming `diagram_id`, one repeating `birth`, and one omitting `death` MUST each raise, and MUST do so on an `arr` whose values would construct cleanly under the positional reading, since a check that fires only on data that fails I2 or I3 is §3.1 catching it rather than this clause. |
| `N11.2-13` | §11.2 | **MUST** | `content_hash` (§8.1, §8.2) MUST be tested against the byte layout this document specifies, not against whatever the implementation currently emits, and the test MUST cover both paths an implementation may take to produce those bytes. |
| `NA.7 `array-api-compat` — what it costs, and what it still corrects-1` | §A.7 `array-api-compat` — what it costs, and what it still corrects | **MUST** | Resolution MUST therefore go through exactly one function. |
| `NB.4 The coefficient-field argument (§8, D17)-1` | §B.4 The coefficient-field argument (§8, D17) | **MUST** | The field occurred exactly once in the 1,731-line draft as it then stood, in that comment: no section, no MUST clause and no test requirement mentioned it. |
| `NB.4 The coefficient-field argument (§8, D17)-2` | §B.4 The coefficient-field argument (§8, D17) | **MUST** | The prose seven lines below said the opposite, "All fields are optional", and the MUST-populate list named three fields that deliberately excluded it. |
